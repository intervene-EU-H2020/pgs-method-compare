...
## Pairwise comparison leaderboard
...
table_columns = {
  if (use_relative === "relative") {
    return ["relative_rank", "method_x", "method_y", "ancestry", "relative_beta_with_ci"];
  } else {
    return ["rank", "method_x", "method_y", "ancestry", "beta_with_ci"];
  }
}
...
table_header = {
  if (use_relative === "relative") {
    return {
      ...
      relative_beta_with_ci: "Relative difference"
    };
  } else {
    return {
      rank: "Rank",
...
filtered_pairwise = aq.fromJSON(pv_mrg)
  ...
  .filter((d, $) => op.includes($.method_type, d.method_type_y ) &&
    op.includes($.method_type, d.method_type_x ) &&
...
## Absolute effect size leaderboard
