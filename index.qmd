...
You can view the effects sizes of each of the methods in each of the biobanks on the [absolute effect sizes tab](effect-sizes.qmd). To see how methods compare against a baseline method take a look at the [relative effect sizes tab](relative.qmd). Instead of comparing biobanks separately, we can pool information across all of the biobanks and perform a meta-analysis, which improves the fidelity of our absolute and relative effect size benchmarks [(meta analysis tab)](meta.qmd). To get an overview of the top performing methods [check out our leaderboards.](leaderboard.qmd)
